...
class DrawingClassifier:
    ...
    def classes_prompt(self):
        ...
        if os.path.exists(self.proj_name):          #If the project already exists load the data
        ...
        else:           #If not then get the 3 new classes, initialize variables, and create directories
    ...
    def save(self, class_num):          #Saves an image as temp.png, if the image is classified under class x, create or save to that directory
    ...
    def train_model(self):
        img_list = np.array([])         #Numpy array full of pixels
        class_list = np.array([])        #Numpy array full of labels (classes) that go with image

        for x in range(1, self.class1_counter):
            img = cv.imread(f"{self.proj_name}/{self.class1}/{x}.png")[:, :, 0]         #Load the image
            img = img.reshape(2500)         #Flatten array into a 1D vector that is 2500 long
            img_list = np.append(img_list, [img])           #Add image into img list
            class_list = np.append(class_list, 1)           #Add the corresponding class of the img to the class list

        for x in range(1, self.class2_counter):
            img = cv.imread(f"{self.proj_name}/{self.class2}/{x}.png")[:, :, 0]         #Load the image
            img = img.reshape(2500)         #Flatten array into a 1D vector that is 2500 long
            img_list = np.append(img_list, [img])           #Add image into img list
            class_list = np.append(class_list, 2)           #Add the corresponding class of the img to the class list

        for x in range(1, self.class3_counter):
            img = cv.imread(f"{self.proj_name}/{self.class3}/{x}.png")[:, :, 0]         #Load the image
            img = img.reshape(2500)         #Flatten array into a 1D vector that is 2500 long
            img_list = np.append(img_list, [img])           #Add image into img list
            class_list = np.append(class_list, 3)           #Add the corresponding class of the img to the class list
    ...
    def predict(self):
        ...
        img = PIL.Image.open("temp.png")
        img.thumbnail((50,50), PIL.Image.Resampling.LANCZOS)
        img.save("predictshit.png", "PNG")

        img = cv.imread("predictshit.png")[:, :, 0]
        img = img.reshape(2500)
        prediction = self.clf.predict([img])
        if prediction[0] == 1:
            tkinter.messagebox.showinfo("Drawing Classifier", f"Probably a {self.class1}", parent=self.root)
        elif prediction[0] == 2:
            tkinter.messagebox.showinfo("Drawing Classifier", f"Probably a {self.class2}", parent=self.root)
        elif prediction[0] == 3:
            tkinter.messagebox.showinfo("Drawing Classifier", f"Probably a {self.class3}", parent=self.root)

    def rotate_model(self):         #Switch between models
    ...
    def save_model(self):
        ...
        tkinter.messagebox.showinfo("Drawing Classifier", "Model sucessfully saved", parent=self.root)

    def load_model(self):
        file_path = filedialog.askopenfilename()
        with open(file_path, "rb") as f:
            self.clf = pickle.load(f)
        tkinter.messagebox.showinfo("Drawing Classifier", "Model sucessfully loaded", parent=self.root)
    ...
    def save_everything(self):
        ...
        with open(f"{self.proj_name}/{self.proj_name}_data.pickle", "wb") as f:
            pickle.dump(data, f)
        tkinter.messagebox.showinfo("Drawing Classifier", "Project successully saved", parent=self.root)
